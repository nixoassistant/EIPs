...
#### Scheduled for Inclusion Criteria

To move a Core EIP from `Considered for Inclusion` to `Scheduled for Inclusion`, the following criteria **SHOULD** be met:

1. **Devnet Stability**: The EIP has been included in an Upgrade Devnet that has demonstrated stability for at least one week, with high client participation and no critical bugs related to the EIP.

2. **Specification Finalized**: The EIP specification is finalized, with no placeholder values, unresolved design decisions, or pending normative changes.

3. **Minimal Unresolved Interactions**: The EIP has minimal unresolved interactions with other `Considered for Inclusion` EIPs. If interactions exist, they **SHOULD** have been tested together in the same Upgrade Devnet.

4. **Adequate Testing Coverage**: The EIP has adequate testing coverage. Tests in Hive and execution-spec-tests are passing, and there are no known client divergences related to the EIP's behavior.

These criteria are intended to be objective checkpoints. Meeting them does not automatically result in `Scheduled for Inclusion` — client teams **MAY** raise additional concerns on the relevant ACD call. However, when these criteria are met and no objections are raised, the EIP **SHOULD** be moved to `Scheduled for Inclusion`.

#### Decision Process

The decision to move an EIP to `Scheduled for Inclusion` **SHOULD** follow this process:

1. **ACDT Review**: The All Core Devs - Testing (ACDT) call reviews whether an EIP meets the criteria above and recommends it for `Scheduled for Inclusion`.

2. **ACDE/ACDC Decision**: The recommendation is presented on the next ACDE or ACDC call. If no objections are raised, the EIP **SHOULD** be moved to `Scheduled for Inclusion`. This ensures that the broader set of client developers who may not attend ACDT have the opportunity to review and object.

This process avoids blocking `Scheduled for Inclusion` decisions on ACD calls when criteria are objectively met, while preserving the ability for any client team to raise concerns on the record.

### Upgrade Devnet Prioritization

After all `Considered for Inclusion` decisions have been made for a network upgrade, the ACD coordinator **SHOULD** propose a prioritization ordering of CFI EIPs for Upgrade Devnet inclusion. This ordering:

- **SHOULD** take into account client team S-tier rankings, testing team capacity, and EIP complexity.
- **SHOULD** be presented on an ACD call for review, but is not a formal governance decision — it serves as guidance for devnet planning.
- **MAY** be reflected in the Upgrade Meta EIP as an ordered list or numbering.

This prioritization is separate from the `Scheduled for Inclusion` status. Its purpose is to provide a clear signal to the testing team and devnet infrastructure operators about which EIPs to prioritize for implementation and devnet inclusion, filling the gap between `Considered for Inclusion` and `Scheduled for Inclusion`.
